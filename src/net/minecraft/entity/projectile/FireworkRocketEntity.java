        ...
        if (this.isAttachedToEntity())
        {
            ...
            if (this.boostedEntity != null) {
                if (this.boostedEntity.isElytraFlying()) {
                    ...
                    if (Vredux.getInstance().getFunctionRegistry().getElytraBooster().isState()) {
                        boostedSpeed = 1.63;
                    }

                    KillAura KillAura = Vredux.getInstance().getFunctionRegistry().getKillAura();
                    if (KillAura.isState() && KillAura.getOptions().getValueByName("Коррекция движения").get() && boostedEntity instanceof ClientPlayerEntity) {
