...
public class HeadLayer<T extends LivingEntity, M extends EntityModel<T> & IHasHead> extends LayerRenderer<T, M>
{
    ...
    public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch)
    {
        ...
        if (Vredux.getInstance().getFunctionRegistry().getChinaHat().isState() && entitylivingbaseIn instanceof PlayerEntity player && ((player instanceof ClientPlayerEntity) || Vredux.getInstance().getFriendStorage().isFriend(TextFormatting.getTextWithoutFormattingCodes(player.getName().getString())))) {
            ...
            matrixStackIn.push();
            {
                ...
                buffer.begin(GL_TRIANGLE_FAN, POSITION_COLOR);
                {
                    buffer.pos(matrixStackIn.getLast().getMatrix(), 0, .3f, 0)
                            .color(ColorUtils.setAlpha(HUD.getColor(155), 255))
                            .endVertex();
                    for (int i = 0, size = 360; i <= size; i++) {
                        buffer.pos(matrixStackIn.getLast().getMatrix(),
                                ...
                                .color(ColorUtils.setAlpha(HUD.getColor(155), 150))
                                .endVertex();
                    }
                }
                tessellator.draw();
