...
import wtf.sqwezz.events.EventCancelOverlay;
...
public class OverlayRenderer {
    ...
    public static void renderOverlays(Minecraft minecraftIn, MatrixStack matrixStackIn) {
        ...
        if (!playerentity.noClip) {
            if (Reflector.ForgeEventFactory_renderBlockOverlay.exists()
                    && Reflector.ForgeBlockModelShapes_getTexture3.exists()) {
                Pair<BlockState, BlockPos> pair = getOverlayBlock(playerentity);

                if (pair != null) {
                    Object object = Reflector.getFieldValue(Reflector.RenderBlockOverlayEvent_OverlayType_BLOCK);

                    if (!Reflector.ForgeEventFactory_renderBlockOverlay.callBoolean(playerentity, matrixStackIn, object,
                            pair.getLeft(), pair.getRight())) {
                        TextureAtlasSprite textureatlassprite = (TextureAtlasSprite) Reflector.call(
                                minecraftIn.getBlockRendererDispatcher().getBlockModelShapes(),
                                Reflector.ForgeBlockModelShapes_getTexture3, pair.getLeft(), minecraftIn.world,
                                pair.getRight());
                ...
                if (blockstate != null) {
                    renderTexture(minecraftIn,
                            minecraftIn.getBlockRendererDispatcher().getBlockModelShapes().getTexture(blockstate),
                            matrixStackIn);
                }
            }
        }
        ...
        if (!minecraftIn.player.isSpectator()) {
            if (minecraftIn.player.areEyesInFluid(FluidTags.WATER)
                    && !Reflector.ForgeEventFactory_renderWaterOverlay.callBoolean(playerentity, matrixStackIn)) {
                renderUnderwater(minecraftIn, matrixStackIn);
            }

            if (minecraftIn.player.isBurning()
                    && !Reflector.ForgeEventFactory_renderFireOverlay.callBoolean(playerentity, matrixStackIn)) {
    ...
    private static void renderFire(Minecraft minecraftIn, MatrixStack matrixStackIn) {
        ...
        Vredux.getInstance().getEventBus().post(eventCancelOverlay);
