...
import wtf.sqwezz.command.CommandDispatcher;
import wtf.sqwezz.command.impl.DispatchResult;
import wtf.sqwezz.events.*;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            if (!Vredux.getInstance().playerOnServer) {
                Vredux.getInstance().playerOnServer = true;
            }

            Vredux.getInstance().getEventBus().post(eventUpdate);
            super.tick();
    ...
    private void onUpdateWalkingPlayer() {
        ...
        Vredux.getInstance().getEventBus().post(ea);
        ...
        if (this.isCurrentViewEntity()) {
            ...
            Vredux.getInstance().getEventBus().post(eventMotion);
        ...
        Vredux.getInstance().getEventBus().post(new EventPostMotion());
    ...
    public void sendChatMessage(String message) {
        ...
        CommandDispatcher commandDispatcher = Vredux.getInstance().getCommandDispatcher();
    ...
    public void closeScreen() {
        ...
        Vredux.getInstance().getEventBus().post(event);
        if (!event.isCancel()) {
    ...
    public void setSprinting(boolean sprinting) {
        Vredux.getInstance().getEventBus().post(sprintEvent);
    ...
    public void livingTick() {
        ...
        if (this.isHandActive() && !this.isPassenger()) {
            Vredux.getInstance().getEventBus().post(noSlowEvent);
        ...
        FunctionRegistry FunctionRegistry = Vredux.getInstance().getFunctionRegistry();
        AutoSprint autoSprint = FunctionRegistry.getAutoSprint();

        if (autoSprint.isState() && !this.isSprinting() &&
