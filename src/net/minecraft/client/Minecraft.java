...
import wtf.sqwezz.events.EventChangeWorld;
import wtf.sqwezz.events.TickEvent;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        if (s != null) {
            this.displayGuiScreen(new ConnectingScreen(selfDestruct.unhooked ? new MainMenuScreen() : new MainMenuScreen(), this, s, i));
        } else {
            this.displayGuiScreen(selfDestruct.unhooked ? new MainMenuScreen() : new MainMenuScreen());
        }
    ...
    public void setDefaultMinecraftTitle() {
        ...
        if (selfDestruct.unhooked) {
            ...
            this.mainWindow.setWindowTitle("Crysense 1.16.5 ");
        }
    }
    ...
    public void displayGuiScreen(@Nullable Screen guiScreenIn) {
        ...
        if (guiScreenIn == null && this.world == null) {
            guiScreenIn = selfDestruct.unhooked ? new MainMenuScreen() : new MainMenuScreen();
    ...
    private void tick(boolean isDebug, @Nullable LongTickDetector detector) {
        Vredux.getInstance().getEventBus().post(tickEvent);
                    ...
                    if (this.objectMouseOver != null) {
                        switch (this.objectMouseOver.getType()) {
                            case ENTITY -> {
                                ...
                                boolean killAuraIsStateAndTargetNotNull = killAura.isState() && killAura.getTarget() != null;

                                if (!killAuraIsStateAndTargetNotNull) {
    ...
    public void runTick() {
        ...
        if (this.world != null) {
            Vredux.getInstance().getEventBus().post(player.tick);
    ...
    public void loadWorld(ClientWorld worldClientIn) {
        ...
        Vredux.getInstance().getEventBus().post(new EventChangeWorld());
    }
