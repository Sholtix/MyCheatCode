...
import wtf.sqwezz.utils.math.MathPenis;
...
public class IngameGui extends AbstractGui {
    ...
    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        ...
        if (eventDisplay.getType() == EventDisplay.Type.PRE) {
            Vredux.getInstance().getEventBus().post(eventDisplay);
        }
        ...
        if(Vredux.getInstance().interpolateState > 0.97)
            Vredux.getInstance().needValueInterpolate = 0;
        if(Vredux.getInstance().interpolateState < 0.02)
            Vredux.getInstance().needValueInterpolate = 1;
        Vredux.getInstance().interpolateState = Animation.fast(Vredux.getInstance().interpolateState, Vredux.getInstance().needValueInterpolate, 4);
        ...
        eventDisplay.setMatrixStack(matrixStack);
        eventDisplay.setPartialTicks(partialTicks);
        SqwezzBubenci.NOTIFICATION_MANAGER.draw(matrixStack);
        mc.gameRenderer.setupOverlayRendering(2);
