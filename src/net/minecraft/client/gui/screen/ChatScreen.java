...
public class ChatScreen extends Screen {
    ...
    protected void init() {
        ...
        this.addButton(button = new Button(mainWindow.getScaledWidth() - 152, mainWindow.getScaledHeight() - 35, 150, 20, new StringTextComponent("Скрыть информацию: " + hide), (button) ->
        {
            hide = !hide;
        }));
    ...
    public void onClose() {
        ...
        for (Dragging dragging : DragManager.draggables.values()) {
            if (dragging.getFunction().isState()) {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        if (button != null) {
            button.setMessage(new StringTextComponent("Скрыть информацию: " + hide));
        }
