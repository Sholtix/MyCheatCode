...
import wtf.sqwezz.events.EventMouseButtonPress;
...
public class MouseHelper {
    ...
    private void mouseButtonCallback(long handle, int button, int action, int mods) {
        if (handle == this.minecraft.getMainWindow().getHandle()) {
            ...
            if (!aboolean[0] && (this.minecraft.currentScreen == null || this.minecraft.currentScreen.passEvents)
                ...
                if (flag) {
                    Vredux.getInstance().onKeyPressed(-100 + i);

                    eventMouseButtonPress.setButton(i);
                    Vredux.getInstance().getEventBus().post(eventMouseButtonPress);
     ...
     *
     * <p>
     * The callback function receives the cursor position, measured in screen
     * coordinates but relative to the top-
     * left corner of the window client area. On platforms that provide it, the full
     * sub-pixel cursor position is passed
     * on.
     * </p>
     *
    ...
    public void updatePlayerLook() {
        ...
        if (this.isMouseGrabbed() && this.minecraft.isGameFocused()) {
            ...
            if (this.minecraft.player != null) {
                ...
                Vredux.getInstance().getEventBus().post(eventRotate);
