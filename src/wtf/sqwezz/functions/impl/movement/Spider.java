...
public class Spider extends Function {
    ...
    private void onMotion(EventMotion motion) {
        switch (mode.get()) {
            ...
            case "Grim" -> {
             ...
             //   if (!mc.player.collidedHorizontally) {
              //      Vredux.getInstance().getFunctionRegistry().getKillAura().canAttackWithSpider = true;
              //      return;
              //  }

                //Vredux.getInstance().getFunctionRegistry().getKillAura().nextAttackDelay = 10;
               // Vredux.getInstance().getFunctionRegistry().getKillAura().canAttackWithSpider = false;
               // Vredux.getInstance().getFunctionRegistry().getKillAura().lastPosY = 0;
    ...
    public void onDisable() {
       // Vredux.getInstance().getFunctionRegistry().getKillAura().nextAttackDelay = 0;
       // Vredux.getInstance().getFunctionRegistry().getKillAura().lastPosY = 0;
      //  Vredux.getInstance().getFunctionRegistry().getKillAura().canAttackWithSpider = true;
        super.onDisable();
    }
