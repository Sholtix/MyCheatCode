...
import static wtf.sqwezz.utils.render.DisplayUtils.drawRoundedRect;
...
public class ESP extends Function {
    ...
    private final ModeSetting typeBox = new ModeSetting("Тип ", "Боксы", "Боксы", "Углы");
    public ESP() {
    ...
    public void onDisplay(EventDisplay e) {
            ...
            if (entry.getKey() instanceof LivingEntity entity) {
                ...
                if (remove.getValueByName("Полоску хп").get()) {
                    ...
                    if (FriendStorage.isFriend(entity.getName().getString())){
                        DisplayUtils.drawMCVerticalBuilding(position.x - hpOffset, position.y + (position.w - position.y) * (1 - MathHelper.clamp(hp / maxHp, 0, 1)), position.x - hpOffset + 1, position.w, getHealthColor(entity, new java.awt.Color(0, 128, 6).getRGB(), new java.awt.Color(0, 128, 6).getRGB()), getHealthColor(entity, new java.awt.Color(0, 128, 6).getRGB(), new java.awt.Color(0, 128, 6).getRGB()));
                    }else {
                        DisplayUtils.drawMCVerticalBuilding(position.x - hpOffset, position.y + (position.w - position.y) * (1 - MathHelper.clamp(hp / maxHp, 0, 1)), position.x - hpOffset + 1, position.w, ColorUtils.rgb(255, 0, 0),ColorUtils.rgb(113, 247, 106));
                    }
                }
            }
        }
        ...
        for (Map.Entry<Entity, Vector4f> entry : positions.entrySet()) {
            ...
            if (entity instanceof LivingEntity living) {
                ...
                if(FriendStorage.isFriend(entity.getName().getString())) {
                    drawRoundedRect(position.x - 2 + width / 2.0F - length / 1.9f, position.y - 16, length + 8, 14, 2, ColorUtils.rgba(0, 255, 0, 88));
                }else {
                    drawRoundedRect(position.x - 2 + width / 2.0F - length / 1.9f , position.y - 16, length + 8, 14, 2, ColorUtils.rgba(21, 21, 21, 150));
                }
...
//                    float length = Fonts.sfbold.getWidth(name,11);
                    ...
                    glCenteredScale(position.x + width / 2f - length / 2f, position.y - 7, length, 10, 0.5f);
                    drawRoundedRect(position.x - 5 + width / 2f - length / 2f, position.y - 16, length + 10, 16, 2, ColorUtils.rgba(10, 10, 10, 150));
