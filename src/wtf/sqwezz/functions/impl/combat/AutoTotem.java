...
import wtf.sqwezz.events.EventSpawnEntity;
...
public class AutoTotem extends Function {
    ...
    private final BooleanSetting saveEnchanted = new BooleanSetting("Сохранять чар", true);
    private final BooleanSetting useInElytra = new BooleanSetting("Брать в элитре", true);
    private final SliderSetting elytraHealth = new SliderSetting("Здоровье в элитре", 3.0f, 1.f, 20.f, 0.5f).setVisible(() -> useInElytra.get());
    ...
    private boolean shouldToSwapTotem() {
        ...
        if (useInElytra.get() && isWearingElytra) {
            return mc.player.getHealth() <= elytraHealth.get();
        }
