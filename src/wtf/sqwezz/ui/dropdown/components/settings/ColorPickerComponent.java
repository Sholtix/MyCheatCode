...
public class ColorPickerComponent extends Component {
    ...
    private void renderTextAndColorRect(MatrixStack stack) {
        ...
        // Устанавливаем X координаты для правого края
        this.textX = this.getX() + this.getWidth();
        this.textY = this.getY() + 2;

        this.colorRectX = this.getX() + this.getWidth() - colorRectWidth - padding;
        this.colorRectY = this.getY() + 4 + padding;
        this.colorRectWidth = this.getWidth() - (padding * 2);
        this.colorRectHeight = padding * 2;

        this.pickerX = this.getX() + this.getWidth() - pickerWidth - padding;
        ...
        // Рендерим текст и цветовой прямоугольник
    ...
    public void mouseClick(float mouseX, float mouseY, int mouse) {
        if (MathUtil.isHovered((int) mouseX, (int) mouseY, colorRectX, colorRectY, colorRectWidth, colorRectHeight) && mouse == 1) {
            panelOpened = !panelOpened;
        }

        if (panelOpened) {
            if (MathUtil.isHovered((int) mouseX, (int) mouseY, sliderX - 2, sliderY, sliderWidth + 4, pickerHeight - 12)) {
                draggingHue = true;
            } else if (MathUtil.isHovered((int) mouseX, (int) mouseY, pickerX, pickerY, pickerWidth, pickerHeight)) {
                draggingPicker = true;
            }
        }
