...
import wtf.sqwezz.utils.render.Cursors;
...
public class ModuleComponent extends Component {
    ...
    private void drawOutlinedRect(float mouseX, float mouseY, int color) {
        ...
        if (MathUtil.isInRegion(mouseX, mouseY, getX(), getY(), getWidth(), 20)) {
            if (!hovered) {
    ...
    private void drawText(MatrixStack stack, int color) {
        ...
        if (components.stream().filter(Component::isVisible).count() >= 1) {
            ...
            if (bind) {
                Fonts.montserrat.drawText(stack, bindText, getX() + getWidth() - 6 - Fonts.montserrat.getWidth(bindText, 6, 0.1f), getY() + Fonts.icons.getHeight(6) + 1, ColorUtils.rgb(161, 164, 177), 6, 0.1f);
            } else
