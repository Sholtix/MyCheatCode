...
public class AltWidget implements IMinecraft {
    ...
    public void updateScroll(int mouseX, int mouseY, float delta) {
        ...
        if (MathUtil.isInRegion(mouseX, mouseY, this.x, this.y, 145, 100) && open) {
            scrollPre += delta * 10;
        }
    }
    ...
    public void render(MatrixStack stack, int x, int y) {
        ...
        scroll = MathUtil.fast(scroll, scrollPre, 3);
        ...
        DisplayUtils.drawShadow(this.x, this.y, width, height, 10, ColorUtils.rgba(0, 0, 0, 115));

        DisplayUtils.drawRoundedRect(this.x, this.y, width, height, new Vector4f(4, 4, 4, 4), ColorUtils.rgba(255, 255, 255, 22));
        Scissor.push();
    ...
    public void click(int mouseX, int mouseY, int button) {
        ...
        if (MathUtil.isInRegion(mouseX, mouseY, this.x, this.y, width, 20)) {
            open = !open;
        ...
        if (!MathUtil.isInRegion(mouseX, mouseY, this.x, this.y, width, 50)) {
            typing = false;
        }
        ...
        if (open) {
            ...
            for (Alt alt : alts) {
                ...
                if (MathUtil.isInRegion(mouseX, mouseY, this.x + 5, this.y + 26 + i * 17 + scroll, width - 10, 15)) {
                    if (button == 0) {
                        AltConfig.updateFile();
        ...
        if (MathUtil.isInRegion(mouseX, mouseY, this.x + 82, this.y + 26 + alts.size() * 17 + 2 + scroll, 10, 10)) {
            if (altName.length() >= 3) {
                alts.add(new Alt(altName));
                AltConfig.updateFile();
            }
        ...
        if (MathUtil.isInRegion(mouseX, mouseY, this.x + 5, this.y + 26 + alts.size() * 17 + scroll, width - 10, 15)) {
            typing = !typing;
        }
    }
