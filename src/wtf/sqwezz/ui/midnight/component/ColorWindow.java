package wtf.sqwezz.ui.midnight.component;
...
public class ColorWindow {
    ...
    public void draw(MatrixStack stack, int mouseX, int mouseY) {
        ...
        DisplayUtils.drawCircle(circleX + 1, circleY + 1, 6, ColorUtils.reAlphaInt(component.option.get(), 255));

        for (int i = 0; i < width - 12; i++) {
            float hue = i / (width - 12);
            DisplayUtils.drawCircle(x + 6 + i, y + width + 6, 6, ColorUtils.reAlphaInt(Color.HSBtoRGB(hue, 1, 1), 255));
        }
        for (int i = 0; i < width - 12; i++) {
            float hue = i / (width - 12);
            DisplayUtils.drawCircle(x + 6 + i, y + width + 18, 6, ColorUtils.interpolateColor(ColorUtils.reAlphaInt(component.option.get(), 255), ColorUtils.IntColor.rgba(17, 18, 21, 255), 1 - hue));
        }
        DisplayUtils.drawCircle(x + 6 + alpha * (width - 12), y + width + 18, 8, Color.BLACK.getRGB());
        DisplayUtils.drawCircle(x + 6 + alpha * (width - 12), y + width + 18, 6, ColorUtils.interpolateColor(ColorUtils.IntColor.rgba(17, 18, 21, 255), ColorUtils.reAlphaInt(component.option.get(), 255), alpha));
        if (draggingHue) {
        ...
        if (dragging || draggingAlpha || draggingHue)
            component.option.set(ColorUtils.reAlphaInt(Color.getHSBColor(hsb[0], hsb[1], hsb[2]).getRGB(), (int) (alpha * 255)));
        else {
            //float[] rgb = ColorUtils.IntColor.rgb(component.option.get());
            //hsb = Color.RGBtoHSB((int) rgb[0], (int) rgb[1], (int) rgb[2], null);
            //alpha = rgb[3] / 255f;
    ...
    public boolean click(int mouseX, int mouseY) {
        ...
        if (MathUtil.isInRegion(mouseX, mouseY, x + 4, y + 321 / 2f - 36 / 2f - 4, 102 / 2f, 36 / 2f)) {
            float[] rgb = ColorUtils.IntColor.rgb(component.option.get());
            ColorWindow.copied[0] = hsb[0];
            ColorWindow.copied[1] = hsb[1];
            ColorWindow.copied[2] = hsb[2];
            return false;
        }
        if (MathUtil.isInRegion(mouseX, mouseY, x + 4 + 51 + 4, y + 321 / 2f - 36 / 2f - 4, 102 / 2f, 36 / 2f)) {
            if (ColorWindow.copied.length >= 3) {
                hsb[0] = ColorWindow.copied[0];
                hsb[1] = ColorWindow.copied[1];
                hsb[2] = ColorWindow.copied[2];
                component.option.set(ColorUtils.reAlphaInt(Color.getHSBColor(hsb[0], hsb[1], hsb[2]).getRGB(), (int) (alpha * 255)));
                onConfigUpdate();
            }
            float[] rgb = ColorUtils.IntColor.rgb(component.option.get());
