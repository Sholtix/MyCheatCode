...
public class SelectorUI implements IMinecraft {
    ...
    public void mouseClicked(double mouseX, double mouseY, int button) {
        ...
        // нажал на крестик
        if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + width - 12, y + 2, 10, 10)) {
            //instance = null;
        ...
        if (Block.getBlockFromItem(selected) instanceof ShulkerBoxBlock b) {
            if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + 8 - 2, y + 23 + 28 - 2 + 0.5f + 17, Fonts.montserrat.getWidth(items ? "Да" : "Нет", 6, 0.03f) + 4, 5 + 4)) {
                items = !items;

            }
            if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + 8 - 2, y + 23 + 28 - 2 + 0.5f, Fonts.montserrat.getWidth(ench ? "Да" : "Нет", 6, 0.03f) + 4, 5 + 4)) {
                don = !don;
            }
        } else {
            if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + 8 - 2, y + 23 + 28 - 2 + 0.5f, Fonts.montserrat.getWidth(ench ? "Да" : "Нет", 6, 0.03f) + 4, 5 + 4)) {
            ...
            if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + 8 - 2, y + 23 + 28 - 2 + 0.5f + 17, Fonts.montserrat.getWidth(fake ? "Да" : "Нет", 6, 0.03f) + 4, 5 + 4)) {
                fake = !fake;
            }
        }
        // Добавил предмет
        if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + 5, y + 100 - 20, 100 - 10, 15)) {
        ...
        if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + 7, y + 30, 50, 11)) {
            priceClicked = !priceClicked;
        }
        if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + 7, y + 30 + 20, 20, 8)) {
            countClicked = !countClicked;
        }
        if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + 7, y + 30 + 11 + 8, 50, 11)) {
            enchantmentUI.stack = instance;
