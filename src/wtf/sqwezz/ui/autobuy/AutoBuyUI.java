...
public class AutoBuyUI extends Screen implements IMinecraft {
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        float height = 300;
        if (selecting) {
            if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x, y, width, height - 65)) {
                sc += (int) (delta * 15);
            }
        } else {
            if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x, y, width, height - 65)) {
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        Fonts.montserrat.drawCenteredText(matrixStack, selecting ? "Назад" : "Добавить", x + width / 2f, y + height - 30 - 10, MathUtil.isInRegion(mouseX, mouseY, x + 10, y + height - 60, width - 20, 50) ? -1 : ColorUtils.rgba(255, 255, 255, 128), 10);
        if (selecting) {
            ...
            for (Item item : Registry.ITEM) {
                ...
                if (MathUtil.isInRegion(itemX, itemY, x, y - 15, width, height - 65 + 15)) {
            ...
            for (AutoBuy buy : Vredux.getInstance().getAutoBuyHandler().items) {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        float width = 400;
        float height = 300;
        if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + 10, y + height - 60, width - 20, 50)) {
            selecting = !selecting;
            if (!selecting) {
        ...
        if (selecting) {
            ...
            int itemY = (int) (y + 5 + scroll);
            {
                int categoryX = (int) (x + width - 100) + 4;
                int categoryY = itemY + 5;
                if (MathUtil.isInRegion((float) mouseX, (float) mouseY, categoryX + 1, categoryY + 1, 13, 13)) {
                    catItems = CatItems.FOOD;
                }
                if (MathUtil.isInRegion((float) mouseX, (float) mouseY, categoryX + 1 + 20, categoryY + 1, 13, 13)) {
                    catItems = CatItems.STONE;
                }
                if (MathUtil.isInRegion((float) mouseX, (float) mouseY, categoryX + 1 + 40 * 0.9f, categoryY + 1, 13, 13)) {
                    catItems = CatItems.SWORD;
                }
                if (MathUtil.isInRegion((float) mouseX, (float) mouseY, categoryX + 1 + 60 * 0.9f, categoryY + 1, 13, 13)) {
                    catItems = CatItems.BOOTS;
                }
                if (MathUtil.isInRegion((float) mouseX, (float) mouseY, categoryX + 1 + 80 * 0.9f, categoryY + 1, 13, 13)) {
                    catItems = CatItems.ALL;
                }
            }
            if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + 5, itemY, width - 100, 28)) {
                searching = !searching;
            }
            itemY += 30;
            for (Item item : Registry.ITEM) {
                ...
                if (MathUtil.isInRegion((float) mouseX, (float) mouseY, itemX, itemY, 15, 15) && MathUtil.isInRegion((float) mouseX, (float) mouseY, x, y, width, height - 65)) {
                    boolean ignore = false;
                    for (AutoBuy a : Vredux.getInstance().getAutoBuyHandler().items) {
                        if (a.getItem() == item) {
                            ignore = true;
                            break;
                        }
            ...
            float offset = 10;
            for (AutoBuy buy : Vredux.getInstance().getAutoBuyHandler().items) {
                if (MathUtil.isInRegion((float) mouseX, (float) mouseY, x + 10 + buyX, y + 10 + buyY, widthPanel, 50) && button == 1) {
                    Vredux.getInstance().getAutoBuyHandler().items.remove(buy);
