...
public class AutoBuyConfig implements IMinecraft {
    ...
    private static final File file = new File(mc.gameDir, "\\Neverlose\\files\\autoBuy.cfg");
    ...
    public static void updateFile() {
        ...
        for (AutoBuy autoBuy : Vredux.getInstance().getAutoBuyHandler().items) {
    ...
    private void readBuyConfig() throws FileNotFoundException {
        ...
        if (jsonObject.has("autoBuys")) {
            for (JsonElement element : jsonObject.get("autoBuys").getAsJsonArray()) {
                ...
                Vredux.getInstance().getAutoBuyHandler().items.add(new AutoBuy(item, price,autoBuyObject.get("count").getAsInt(), enchantments,
