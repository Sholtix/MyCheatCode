...
public class Config implements IMinecraft {
    ...
    public Config(String name) {
        this.name = name;
        this.file = new File(new File(Minecraft.getInstance().gameDir, "\\Neverlose\\configs"), name + ".cfg");
    }
    ...
    private void loadFunctionSettings(JsonObject functionsObject) {
        Vredux.getInstance().getFunctionRegistry().getFunctions().forEach(f -> {
    ...
    private void saveFunctionSettings(JsonObject functionsObject) {
        Vredux.getInstance().getFunctionRegistry().getFunctions().forEach(module -> {
    ...
    private void saveStyleSettings(JsonObject stylesObject) {
        for (Style style : Vredux.getInstance().getStyleManager().getStyleList()) {
            ...
            styleObject.addProperty("selected", Vredux.getInstance().getStyleManager().getCurrentStyle() == style);
