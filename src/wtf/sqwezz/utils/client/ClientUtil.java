...
public class ClientUtil implements IMinecraft {
    ...
    public synchronized void playSound(String sound, float value, boolean nonstop) {
        new Thread(() -> {
            ...
            try {
                ...
                InputStream is = mc.getResourceManager().getResource(new ResourceLocation("Vredux/sounds/" + sound + ".wav")).getInputStream();
    ...
    public static void MainMenu() {
        ...
        if (isMainMenuOpen) {
            if (!hasCheckedMM) {
                System.out.println("MainMenu music - on"); //DEBUG
                volumeMM = 90;
            ...
            if (hasCheckedMM) {
                System.out.println("MainMenu music - off"); //DEBUG
                volumeMM = 0;
